...
class SSD(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        pred_logits = [permute_to_N_HWA_K(x, self.num_classes + 1) for x in pred_logits]
    ...
    def losses(self, anchors, pred_logits, gt_labels, pred_anchor_deltas, gt_boxes):
        ...
        batch_size = pred_logits[0].shape[0]

        num_images = len(gt_labels)

        gt_labels = torch.stack(gt_labels)  # (N, R)
        anchors = type(anchors[0]).cat(anchors).tensor  # (R, 4)
        gt_anchors_deltas = [self.box2box_transform.get_deltas(anchors, k) for k in gt_boxes]
        gt_anchors_deltas = torch.stack(gt_anchors_deltas)  # (N, R, 4)

        valid_mask = gt_labels >= 0
        pos_mask = (gt_labels >= 0) & (gt_labels != self.num_classes)
        neg_mask = (gt_labels == self.num_classes)
        num_pos_anchors  = pos_mask.sum().item()
        num_valid_mask = valid_mask.sum().item()
        ...
        if self.loss_name == "Focal":
            ...
            softmax_ce = F.cross_entropy(pred_logits[valid_mask],
                                         gt_labels[valid_mask],
                                         reduction = 'none')

            loss_for_all_indices = torch.zeros_like(pred_logits[:, :, 1])
            loss_for_all_indices[valid_mask] = softmax_ce

            loss_for_neg_indices = torch.zeros_like(loss_for_all_indices)
            loss_for_neg_indices[neg_mask] = loss_for_all_indices[neg_mask]
            ...
            num_foreground_per_img = pos_mask.view(batch_size, -1).sum(1, keepdim = True)
            ...
            hard_negative = hard_negative & neg_mask

            loss_for_all_indices[~ (hard_negative | pos_mask)] = 0

            loss_cls = loss_for_all_indices[valid_mask].sum(0, keepdims = False) / max(1, num_pos_anchors)

        # regression loss
        loss_box_reg = smooth_l1_loss(
            cat(pred_anchor_deltas, dim=1)[pos_mask],
            gt_anchors_deltas[pos_mask],
            beta=self.smooth_l1_loss_beta,
            reduction="sum",
        ) / max(1, num_pos_anchors)
